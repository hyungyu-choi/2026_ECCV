...
class PermutationClassificationHead(nn.Module):
    """
    MODIFIED from TemporalHead for binary classification.
    Converts backbone features [B,T,D] -> logits [B, 2]
    Class 0: Incorrect Order (shuffled)
    Class 1: Correct Order (chronological)
    """
    def __init__(self, backbone_dim: int, hidden_mul: float = 0.5, max_len: int = 64):
        super().__init__()
        hidden_dim = int(backbone_dim * hidden_mul)

        # 1. Reduce dimension (same as original)
        self.reduce = nn.Sequential(
            nn.Linear(backbone_dim, hidden_dim),
            nn.GELU()
        )
        
        # 2. Process sequence (same as original)
        self.temporal = TemporalSideContext(hidden_dim, max_len=max_len)
        
        # 3. Classifier (MODIFIED)
        # This takes the single aggregated vector and predicts 2 classes
        self.classifier = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.GELU(),
            nn.Linear(hidden_dim // 2, 2) # Output 2 logits for CrossEntropyLoss
        )

    def forward(self, x: torch.Tensor):     # x : [B,T,D]
        x = self.reduce(x)                  # [B,T,hidden_dim]
        x = self.temporal(x)                # [B,T,hidden_dim]
        
        # --- NEW AGGREGATION STEP ---
        # Aggregate the sequence of T features into a single vector.
        # Mean pooling is a simple and standard way to do this.

...
loader_global.py
================
Original loader.py + sampling_mode parameter added to Temporal_RandStep_dataset.
This allows creating two separate dataset instances:
  1) Temporal_RandStep_dataset(..., sampling_mode='randstep')  → original local temporal
  2) Temporal_RandStep_dataset(..., sampling_mode='global')    → global temporal (8-segment)

All other classes (Dataset_puzzle, ImageFolderMask_puzzle, etc.) are unchanged.
"""
...
class Dataset_puzzle(VisionDataset):
    ...
    def __getitem__(self, index: int):
        ...
        frame_name = img_key.decode('utf-8')
...
class Temporal_RandStep_dataset(Dataset):
    """
    Temporal dataset with two sampling modes:
      - 'randstep' (default): original fixed-step local sampling
    ...
    def __init__(
        ...
        # Group inherited keys by vid -> sorted list of integer frame indices
        ...
        # Build samples for epoch 0
        self._build_samples(seed=0)
        print(f"[Temporal Dataset] sampling_mode='{self.sampling_mode}', "
              f"seq_len={self.seq_len}, total samples={len(self.samples)}")
    ...
    def _build_samples(self, seed: int):
        ...
        for vid, idx_list in self.vid2frames.items():
            ...
            # ===== Original randstep sampling mode =====
            else:
